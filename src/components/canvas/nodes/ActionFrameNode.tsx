  ...
  return (
    <div className="relative w-full h-full select-none">
      ...
      <div className="absolute -top-6 left-0 flex items-center gap-1.5 text-slate-400">
        <FrameIcon size={12} className="text-indigo-400 shrink-0" />
        {isEditingTitle ? (
          <input
            type="text"
            value={titleText}
            ...
            autoFocus
            data-nodrag
            className="bg-slate-950 border border-indigo-500/50 rounded px-1 text-[11px] font-semibold text-slate-200 focus:outline-none"
          />
        ) : (
          <span
            onDoubleClick={() => setIsEditingTitle(true)}
            className="text-[11px] font-semibold tracking-wide cursor-pointer hover:text-indigo-300 transition-colors"
      ...
      <div className="w-full h-full rounded-xl border-2 border-dashed border-indigo-500/30 bg-indigo-500/[0.03] transition-colors hover:border-indigo-500/50" />
      ...
      <div
        ...
        {...(bindResize() as any)}
      >
        <svg width="8" height="8" viewBox="0 0 8 8" className="text-indigo-500/60 hover:text-indigo-400">
